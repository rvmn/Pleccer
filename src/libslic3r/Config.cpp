...
ConfigSubstitutions ConfigBase::load_from_gcode_file(const std::string &file, ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    bool has_delimiters = false;
    {
        ...
        static constexpr const char pleccer_gcode_header[] = "; generated by Pleccer";
        ...
        while (std::getline(ifs, header)) {
            if (strncmp(prusaslicer_gcode_header, header.c_str(), strlen(prusaslicer_gcode_header)) == 0
                    || strncmp(pleccer_gcode_header, header.c_str(), strlen(pleccer_gcode_header)) == 0
        ...
        if (! header_found)
            throw Slic3r::RuntimeError("Not a Slic3r/ Pleccer / SuperSlicer / PrusaSlicer generated g-code.");
    }
